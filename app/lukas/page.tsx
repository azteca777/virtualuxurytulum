...
// === COMPONENTE BOTÓN ===
const Button = ({ children, variant = 'primary', className = '', onClick, disabled = false }: any) => {
  const baseClass = "px-4 py-2 md:px-6 md:py-3 text-[10px] md:text-sm font-extrabold uppercase tracking-widest transition-all duration-300 transform shadow-md";
  const hoverClass = disabled ? "opacity-50 cursor-not-allowed" : "hover:-translate-y-1 hover:shadow-xl";
  const variants = {
    primary: "bg-black text-white hover:bg-zinc-800",
    secondary: "bg-zinc-200 text-black hover:bg-white hover:text-black border border-zinc-300",
    danger: "bg-red-600 text-white hover:bg-red-700"
  };
  return <button onClick={onClick} disabled={disabled} className={`${baseClass} ${hoverClass} ${variants[variant as keyof typeof variants]} ${className}`}>{children}</button>;
};
  ...
  <div className="border border-zinc-100 bg-white p-3 lg:p-4 group hover:shadow-2xl hover:border-red-600 transition-all duration-500 flex flex-col relative">
    ...
    <div className="flex-grow flex flex-col justify-between">
      ...
      <div>
        ...
        <Button 
          variant="primary" 
          className={`w-full text-[9px] lg:text-[11px] py-2`}
          disabled={producto.tag === 'Agotado'}
...
export default function LukasStore() {
  ...
  // === ESTADOS DEL CARRITO, IDIOMA Y PAGOS ===
  ...
  const generarLinkDePago = async () => {
    ...
    try {
      // 🚨 CONECTADO A PRODUCCIÓN: Apunta a tu dominio real de ViOs Code
      ...
      if (data.urlPago) {
        // Redirigimos al cliente a la página segura de cobro
        window.location.href = data.urlPago;
      } else {
        alert('Error en la Matriz: ' + (data.error || 'Desconocido'));
        setProcesandoPago(false);
      }
    } catch (error) {
      console.error('Error de conexión:', error);
      alert('Fallo al conectar con ViOs Code. Asegúrate de tener la Matriz activa.');
      setProcesandoPago(false);
    }
  };
  ...
  return (
    <div className="min-h-screen bg-white text-black font-sans selection:bg-red-600 selection:text-white relative">
            ...
            <div className="w-full md:w-1/2 p-6 md:p-8 flex flex-col justify-center">
              ...
              <div className="flex gap-4">
                <Button 
                  ...
                  disabled={productoSeleccionado.tag === 'Agotado'}
                  onClick={() => {
            ...
            {carrito.length > 0 && (
              <div className="p-6 bg-zinc-50 border-t border-zinc-200">
                ...
                <p className="text-[10px] text-center text-zinc-500 mt-4 px-4 leading-relaxed">
                  {idioma === 'es' 
                    ? 'Serás redirigido a un entorno seguro para procesar tu pago.'
                    : 'You will be redirected to a secure environment to process your payment.'}
                </p>
              </div>
            )}
          </div>
      ...
      <a 
        href="https://wa.me/5219984760310?text=%C2%A1Hola!%20Tengo%20una%20duda%20sobre%20los%20suplementos%20de%20Gods%20Aesthetics.%20%F0%9F%92%AA" 
        target="_blank" 
      ...
      >
        ...
        <span className="absolute right-14 bg-black text-white text-[10px] px-3 py-1 rounded-full opacity-0 group-hover:opacity-100 transition-opacity whitespace-nowrap pointer-events-none">
          {idioma === 'es' ? '¿Dudas? Escríbenos' : 'Questions? Chat'}
        </span>
      </a>
