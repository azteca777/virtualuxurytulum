...
function EntornoVideo360({ urlVideo, rotacion = [0, 0, 0] }: { urlVideo: string, rotacion?: [number, number, number] }) {
  ...
  return (
    <mesh rotation={rotacion} scale={[-1, 1, 1]}>
...
// === NAVEGACIÓN (Flechas y Texto Flotante R3F) ===
function FlechaStreetView({ posicion, angulo = 0, onClick, tipo = 'continuar' }: { posicion: [number, number, number], angulo?: number, onClick: () => void, tipo?: 'continuar' | 'regresar' }) {
  const [hovered, setHovered] = useState(false);
  const textRef = useRef<THREE.Group>(null!);

  useFrame((state) => {
    if (textRef.current) {
      textRef.current.position.y = 0.8 + Math.sin(state.clock.elapsedTime * 3) * 0.1;
    }
  });
  ...
  return (
    ...
    >
      ...
      <mesh position={[0, 0.3, 0.02]} rotation={[0, 0, Math.PI / 2]}>
        ...
        <meshBasicMaterial color="#EAB308" transparent opacity={hovered ? 1 : 0.8} /> 
      </mesh>

      <group ref={textRef} rotation={[Math.PI / 2, 0, -angulo]}>
        <Html center style={{ pointerEvents: 'none' }}>
          <div className={`px-4 py-1.5 font-mono text-[10px] rounded-full border tracking-widest uppercase transition-all duration-300 ${hovered ? 'bg-[#EAB308] text-black border-[#EAB308] scale-110 font-bold shadow-[0_0_15px_rgba(234,179,8,0.5)]' : 'bg-black/80 text-[#EAB308] border-[#EAB308]/30 font-bold'}`}>
            {tipo === 'continuar' ? 'CONTINUAR ➔' : '← REGRESAR'}
          </div>
        </Html>
      </group>
    </group>
...
// === PANTALLA DE VIDEO FLOTANTE (NUEVO) ===
function PantallaVideo({ urlVideo, posicion, rotacion, tamano }: { urlVideo: string, posicion: [number, number, number], rotacion: [number, number, number], tamano: [number, number] }) {
  const [hovered, setHovered] = useState(false);
  const texturaVideo = useVideoTexture(urlVideo, { muted: true, loop: true, start: true });
  ...
  return (
    ...
    >
      <mesh>
        <planeGeometry args={tamano} />
        <meshBasicMaterial map={texturaVideo} side={THREE.DoubleSide} />
      </mesh>

      {hovered && (
        <Html center position={[0, -(tamano[1] / 2) - 0.5, 0]} style={{ pointerEvents: 'none' }}>
          <div className="px-4 py-2 bg-[#8B5CF6] text-white font-bold text-[10px] rounded-full tracking-widest uppercase shadow-[0_0_15px_rgba(139,92,246,0.6)] whitespace-nowrap animate-pulse">
            AGENDAR SESIÓN ➔
          </div>
        </Html>
      )}
  ...
  return (
    <Canvas camera={{ position: [0, 0, 0.1], fov: 70 }}>
      ...
      <Suspense fallback={
        ...
        {nodoActual === 1 && (
          <group>
            <EntornoVideo360 urlVideo={URL_ENTRADA} rotacion={[0, -Math.PI / 0.1, 0]} />
            <FlechaStreetView posicion={[-3, -2.5, -3]} onClick={() => setNodoActual(2)} tipo="continuar" />
          </group>
        )}
        ...
        {nodoActual === 2 && (
          <group>
            ...
            <PantallaVideo 
              urlVideo="https://grutgbujoy4xc00c.public.blob.vercel-storage.com/reservas_meta.mp4" 
              posicion={[-6.5, 1.5, -2.5]}  
              rotacion={[0, Math.PI / 2, 0]} 
              tamano={[3, 1.5]} 
            />

            <FlechaStreetView posicion={[3, -4, -7]} angulo={Math.PI} onClick={() => setNodoActual(1)} tipo="regresar" />
            <FlechaStreetView posicion={[-1, -4, 8]} onClick={() => setNodoActual(3)} tipo="continuar" />
          </group>
        )}
        ...
        {nodoActual === 3 && (
          <group>
            ...
            <FlechaStreetView posicion={[6, -4, -8]} angulo={Math.PI} onClick={() => setNodoActual(2)} tipo="regresar" />
            <FlechaStreetView posicion={[-4, -4, 6]} onClick={() => setNodoActual(4)} tipo="continuar" />
          </group>
        )}
        ...
        {nodoActual === 4 && (
          <group>
            ...
            <FlechaStreetView posicion={[-5, -4, 7]} angulo={Math.PI} onClick={() => setNodoActual(3)} tipo="regresar" />
          </group>
        )}
         ...
         <div className="max-w-5xl mx-auto text-center">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6 text-left">
              {[
                ...
                { slug: 'linea-fina-botanico', title: idioma === 'es' ? 'LÍNEA FINA Y BOTÁNICO' : 'FINE LINE & BOTANICAL', desc: idioma === 'es' ? 'Trazos ultra finos, precisión milimétrica y composiciones orgánicas. Delicadeza visual con la fuerza del blackwork para piezas elegantes y atemporales.' : 'Ultra-fine strokes, pinpoint precision, and organic compositions. Visual delicacy with the strength of blackwork for elegant, timeless pieces.', icon: '🌿' },
      ...
      <footer id="contacto" className="bg-[#0a0a0a] pt-20 pb-10 px-6 border-t border-[#222]">
        <div className="max-w-6xl mx-auto grid grid-cols-1 md:grid-cols-4 gap-12 mb-16">
          ...
          <div>
            ...
            <div 
              onClick={() => window.open('https://maps.app.goo.gl/NkQsCa1jgZA8bLUK9?g_st=iw', '_blank')}
              className="w-full h-32 rounded-lg overflow-hidden border border-[#333] shadow-[0_0_15px_rgba(139,92,246,0.2)] hover:border-[#8B5CF6] transition-colors relative group cursor-pointer"
            >
              {/* Esta capa invisible (z-10) es la que captura el clic para llevarte a tu link */}
              <div className="absolute inset-0 z-10"></div>
              
              <iframe 
                src="https://maps.google.com/maps?q=Plaza+Paseo+Coba,Playa+del+Carmen&t=&z=15&ie=UTF8&iwloc=&output=embed" 
                width="100%" 
                height="100%" 
                style={{ border: 0 }} 
              ...
              <div className="absolute top-2 right-2 z-20 opacity-0 group-hover:opacity-100 transition-opacity">
                <span className="bg-[#8B5CF6] text-white text-[8px] font-bold px-2 py-1 rounded uppercase tracking-tighter">
                  Ver en Google Maps ➔
                </span>
              </div>
            </div>
