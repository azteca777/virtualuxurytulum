...
export default function Home() {
  ...
  // --- MOTOR DE CONEXIÓN A VIOS CODE EN VERCEL ---
  const ejecutarPruebaVirtual = async () => {
    if (!fotoUsuarioBase64 || !prendaSeleccionadaUrl) {
      setErrorTryOn('¡Sube tu foto para continuar!');
      return;
    }
    ...
    try {
      // Usa el dominio nativo de Vercel para saltarte las redirecciones
      ...
      if (!response.ok) throw new Error(data.error || 'Error de conexión con ViOs Code.');

      // YouCam devuelve el resultado aquí
  ...
  // Convertimos la foto del usuario a Base64 para enviarla a ViOs Code
  const manejarSubidaFoto = (e: React.ChangeEvent<HTMLInputElement>) => {
    const file = e.target.files?.[0];
    if (file) {
      setErrorTryOn(null);
      const reader = new FileReader();
      reader.onloadend = () => setFotoUsuarioBase64(reader.result as string);
         ...
         <div className="absolute inset-0 pointer-events-none flex flex-col justify-between z-10">
           ...
           <header className="w-full p-6 md:p-10 flex justify-between items-start">
             ...
             <div className="text-right">
               ...
               <p className="font-inter text-yellow-500 text-[9px] md:text-[11px] tracking-[0.3em] uppercase drop-shadow-[0_1px_2px_rgba(0,0,0,0.8)] mt-1">Virtual Luxury Matrix</p>
             </div>
           </header>
           ...
           {prendaSeleccionadaUrl && !resultadoTryOnUrl && (
             <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[90%] md:w-[400px] pointer-events-auto bg-black/80 backdrop-blur-xl p-8 rounded-3xl border border-white/10 shadow-2xl text-white z-20">
               ...
               <div className="mb-6 flex justify-center">
                 <img src={prendaSeleccionadaUrl} alt="Prenda" className="w-24 h-32 object-cover border border-white/10 rounded-lg p-1 bg-white/5"/>
               </div>
               ...
               <div className="mb-6">
                 ...
                 {fotoUsuarioBase64 && <p className="text-green-400 text-[9px] mt-2 uppercase tracking-wider">¡Foto cargada! ✅</p>}
               </div>
           ...
           {resultadoTryOnUrl && (
             <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 pointer-events-auto bg-black/90 backdrop-blur-3xl p-4 rounded-2xl border border-yellow-500/30 z-30 flex flex-col items-center shadow-[0_0_30px_rgba(234,179,8,0.1)]">
               <img src={resultadoTryOnUrl} alt="Resultado" className="max-w-[85vw] max-h-[70vh] rounded-xl object-contain mb-4"/>
               <div className="flex gap-4 w-full">
                 ...
                 <a href={resultadoTryOnUrl} download="Prueba_Virtualuxury.jpg" target="_blank" rel="noreferrer" className="flex-1 py-3 bg-yellow-500 hover:bg-yellow-400 text-black text-center rounded-xl text-[10px] tracking-widest font-bold uppercase transition-colors">Descargar</a>
               </div>
             </div>
           )}

           {/* BOTÓN CON TU TRAJE DE BAÑO EN VERCEL */}
           {!prendaSeleccionadaUrl && !resultadoTryOnUrl && (
